...
class CustomLlamaDecoderLayer(GradientCheckpointingLayer):
    ...
    def __init__(self, config: LlamaConfig, layer_idx: int):
        ...
        self.post_init(config, layer_idx)


    def post_init(self, config, layer_idx):
        # Initialize weights
        return
...
class LlamaPreTrainedModel(PreTrainedModel):
    ...
    _can_record_outputs = {
        "hidden_states": CustomLlamaDecoderLayer,
...
class CustomLlamaModel(LlamaPreTrainedModel):
    ...
    def __init__(self, config: LlamaConfig):
        ...
        self.layers = nn.ModuleList(
            [self.layer_type(config, layer_idx) for layer_idx in range(config.num_hidden_layers)]
        )
